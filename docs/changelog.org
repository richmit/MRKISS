...
:END:
  ...
  - API breaking Changes
    ...
    - sol_no_dy_o & sol_no_t_o arguments removed
      - Replaced by sol_w_dy_o & sol_w_t_o
      - Old arguments were integer, new ones are logical
      - Old argument values were not used, just present().
      - Usage is inverted (went from without to with)
