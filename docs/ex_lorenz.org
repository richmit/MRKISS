...
point tracing the curve speed up and slow down as the derivative changes.  We can produce such a solution with the ~steps_fixed_stab~ solvers. Here is an
...
The solutions used in the images above were created by this snippet of [[https://github.com/richmit/MRKISS/blob/master/examples/lorenz.f90][~examples/lorenz.f90~]]:
...
Of course we can use ~steps_condy_stab~ and ~steps_sloppy_condy_stab(..., adj_short_o=1)~ as we did in the [[#fixed-y][previous section]]; however,
~steps_sloppy_condy_stab~ without the ~adj_short_o=1~ argument that may be more useful.  Without this option only steps longer than the target are adjusted.
