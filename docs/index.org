...
:END:
  ...
  - ~steps_points_stab_*t()~ takes a seto of \(t\) values at which to find solutions ::
...
:END:
  ...
  - ~solution~ :: Array for solution.  \\
    - Each *column* is a solution containing, by default, \(t\), \(\mathbf{y}\), and \(\mathbf{y'}\).
    - By default the first column contains \(t\).  
      - The inclusion of \(t\) may be suppressed via ~sol_w_t_o=0~
    - The coordinates of \(\mathbf{y}\) start in column ~sol_y_idx_o~, by default ~2~. 
    - By default the coordinates \(\mathbf{y'}\) are included, and begin immediatly after the coordinates of \(\mathbf{y}\). 
      - The inclusion of the derivatives may be suppressed via ~sol_w_dy_o=0~.
...
Releases are contained in commits with a tag that looks like this: "vYYYY-MM-DD" -- i.e. a ISO format date prefixed with the letter "v".  These commits differ
from other commits in terms of quality control.  Release commits:
...
That said, I generally try to only push working code to github so head should be reasonably safe.
